...
# Roll all three dice and track their values
func roll_three_d6():
	dice_rolls = []
...
# Select which Table Pile will draw from based on dice roll
func select_draw_wind():
	match number_rolled_dice:
		6, 10, 14, 18:
			GameState.current_draw_wind = "South"
		3, 7, 11, 15:
			GameState.current_draw_wind = "West"
		4, 8, 12, 16:
			GameState.current_draw_wind = "North"
		5, 9, 13, 17:
			GameState.current_draw_wind = "East"
...
func draw_initial_tiles():
	...
	var pile_wind = get_node(GameState.current_draw_wind)
	...
	# Set the first player's turn after initial draw
	GameState.current_player_turn = 1
	GameState.player_turn_changed.emit(1)
...
func draw_tiles(player_number, number_of_tiles = 1, recursion_depth = 0):
	...
	if pile_wind.current_pile_draw_position > pile_wind.max_tile_per_line:
		print("Current position " + str(pile_wind.current_pile_draw_position) + " exceeds max of " + str(pile_wind.max_tile_per_line) + " in " + GameState.current_draw_wind)
		GameState.current_draw_wind = GameState.get_next_wind(GameState.current_draw_wind)
		pile_wind = get_node(GameState.current_draw_wind)
		pile_wind.current_pile_draw_position = 1
		print("Moved to next wind: " + GameState.current_draw_wind + " at position 1")
	
	# Check if this wind has any available tiles
	if not pile_wind.has_available_tiles():
		print("No available tiles in " + GameState.current_draw_wind + ", trying next wind")
		
		# Try all winds in sequence until we find one with tiles
		var original_wind = GameState.current_draw_wind
		...
		while not pile_wind.has_available_tiles() and not tried_all_winds and winds_checked < 4:
			...
			if GameState.current_draw_wind == original_wind or winds_checked >= 4:
				tried_all_winds = true
				print("Checked all winds, no available tiles found")
				return false
				
			if pile_wind.has_available_tiles():
				print("Found available tiles in " + GameState.current_draw_wind)
				# Find first valid position
				var next_position = pile_wind.find_next_available_position(1)
				if next_position != -1:
					pile_wind.current_pile_draw_position = next_position
					print("Setting position to " + str(pile_wind.current_pile_draw_position) + " in " + GameState.current_draw_wind)
	
	# At this point, we have a wind with tiles and a valid position
	print("Drawing from " + GameState.current_draw_wind + " at position " + str(pile_wind.current_pile_draw_position))
	...
	if tiles_drawn > 0:
		...
		# Record the draw in game state for regular gameplay (not initial draw)
		if recursion_depth == 0 and number_of_tiles == 1:
			GameState.set_player_drawn(player_number, true)
		...
		if pile_wind.current_pile_draw_position > pile_wind.max_tile_per_line:
			print("Reached end of " + GameState.current_draw_wind + ", but still need " + str(remaining_tiles_to_draw) + " more tiles")
			GameState.current_draw_wind = GameState.get_next_wind(GameState.current_draw_wind)
			pile_wind = get_node(GameState.current_draw_wind)
			pile_wind.current_pile_draw_position = 1
			print("Moving to next wind: " + GameState.current_draw_wind + " at position 1")
		else:
			print("Continuing to draw from " + GameState.current_draw_wind + " at position " + str(pile_wind.current_pile_draw_position))
		...
		print("Failed to draw from " + GameState.current_draw_wind)
		return false

# Distribute initial tiles to the table
